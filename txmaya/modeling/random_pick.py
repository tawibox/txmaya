...
class RandomPick(QtWidgets.QDialog):
    ...
    def run(cls):
        if not cls.dialog_instance:
            cls.dialog_instance = RandomPick()
        if cls.dialog_instance.isHidden():
            cls.dialog_instance.show()
            cls.dialog_instance.get_list_sel_base()
            cls.dialog_instance.get_count_picked()
            cls.dialog_instance.display_count_sel_base()
            cls.dialog_instance.display_count_picked()
        else:
    ...
    def showEvent(self, e):
        super(RandomPick, self).showEvent(e)

        if self.geometry:
            self.restoreGeometry(self.geometry)

    def closeEvent(self, e):
        if isinstance(self, RandomPick):
            super(RandomPick, self).closeEvent(e)
...
if __name__ == '__main__':
    try:
        RandomPick_test.close()
        RandomPick_test.deleteLater()
    except:
        pass

    RandomPick_test = RandomPick()
    RandomPick_test.show()
    RandomPick_test.get_list_sel_base()
    RandomPick_test.get_count_picked()
    RandomPick_test.display_count_sel_base()
    RandomPick_test.display_count_picked()
